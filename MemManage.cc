...
MemoryManage::_addMemory()
{
	...
	requestMemSize *= 2;
	return std::make_tuple(static_cast<address_type>(address), requestMemSize);
}
...
void MemoryManage::addMemory()
{
	...
	combineAddressSegment(itor, Free_Node(temp_addr, temp_length));			// 这里可不可使用移动语义呢?	&& 移动语义我运用的真的是非常的不熟练
}
...
MemoryManage::ERROR_TYPE MemoryManage::deallocate(address_type address)
{
	...
	// No.1 校验，检查待 deallcoate 的 address 是否为 allocate 申请的内存
